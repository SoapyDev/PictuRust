...
const TEXT: [&str; 22] = [
    ...
    "This program uses the clap, pic, anyhow, image, rayon, jwalk, ravif, rgb, webp and rexiv2 crates.",
    "\n############################ Commands ##############################\n",
    ...
    "--quality <Q> : The quality of the image when converting to Webp. The options are between 1.0 and 100.00. Default is 75.0.",
    ...
    "--keep_metadata <m> : If the metadata should be kept. False will not keep metadata including orientation. Default is true.",
    "\n######################### Your Commands ############################\n",
];
...
fn display_options(param: &Parameters) {
    let mut options: Vec<String> = Vec::with_capacity(16);
    ...
    options.push(format!("Keep metadata : {:?}", param.keep_metadata));
    options.push(format!(
        "\n######################### Your Results #############################\n"
    ));
