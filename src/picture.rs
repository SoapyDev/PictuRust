...
use image::{DynamicImage, GenericImageView, ImageError, ImageReader, ImageResult};
use std::path::Path;
...
impl Picture {
    /// # Errors
    /// Returns an error if the image at `source` cannot be decoded.
...
fn create_img_reader(source: &PathBuf) -> ImageReader<BufReader<File>> {
    ImageReader::open(source).expect("Failed to open File")
}
...
fn get_image(reader: ImageReader<BufReader<File>>) -> ImageResult<DynamicImage> {
    reader.decode()
}
...
fn read_exif(exif: &Exif) -> Option<u32> {
    exif.get_field(Tag::Orientation, In::PRIMARY)
        .map(|orientation| orientation.value.get_uint(0).unwrap())
}
